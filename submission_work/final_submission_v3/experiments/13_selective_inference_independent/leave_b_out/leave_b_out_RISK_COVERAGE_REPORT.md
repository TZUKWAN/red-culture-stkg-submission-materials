...
- AURC/AUGRC 95% intervals use fixed-seed percentile bootstrap; every replicate is retained in `leave_b_out_RISK_COVERAGE_BOOTSTRAP.csv`.
- GOAL-13 point metrics (selective accuracy, error exposure, macro-F1 on accepted, macro-F1 on full denominator) are computed with `code/independent_eval/metrics.py` in `leave_b_out_SELECTIVE_GOAL13_METRICS.csv`.
