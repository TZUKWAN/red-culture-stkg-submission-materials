...
## 补：模型无关路由证明（用户核心要求的直接验证）

三本地升级模型 × 预算 {10,20,30,50}%（leave-B-out 口径，pooled n=261，TEST 排除）：
gpt-oss-20b：0.28→0.35→0.43→0.48→0.60（100% 上限 0.83）；qwen3-8b 同形状
（0.35→0.42→0.48→0.58）。升级段一致率 0.71-0.81 vs 同段基线 0.00-0.16
（8/8 显著为正）——**框架把任何接入模型的有效质量提升 60 个百分点以上**；
能力排序 gpt-oss ≥ qwen3-8b 在全部预算成立（机制形状一致）。
诚实保留：r_hat 对随机路由的部署级优势在 n=261 下不显著（详见
15_MODEL_AGNOSTIC_REPORT.md 的三层读法）。gemma-4-e4b 因 LM Studio 无法
加载（HTTP 400）已按政策跳过并留痕。
